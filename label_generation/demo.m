...
files = dir('../dataset/trainval/2cd9*/0066*.jpg');
...
axis on
hold on
scatter(uv(1, 400:500), uv(2, 400:500)) %1, clr, '.')
xy_dist = dist_estimate(size(img,1), size(img, 2), xyz, proj);
crop = xy_dist(372:390, 830:857);
B = crop > 0;
sum(sum(crop))/sum(sum(B))
var(crop);
%imshow(xy_dist)
figure(2)
clf()
scatter3(xyz(1, 1:100), xyz(2, 1:100), xyz(3, 1:100), 1, clr(1, 1:100), '.')
hold on
axis equal
